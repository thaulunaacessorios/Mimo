...
function initSplash() {
  ...
  if (!splash || !envelope) return;

  /* Clicar no envelope abre e vai para o passo 1 */
  envelope.addEventListener('click', () => {
    envelope.classList.add('open');
    setTimeout(() => closeSplash(splash), 800);
  });

  /* Fallback: fecha automaticamente após 4s sem interação */
  setTimeout(() => {
    if (splash.style.display !== 'none') closeSplash(splash);
  }, 4000);
}
